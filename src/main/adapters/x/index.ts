...
async function template(relearn = false): Promise<RequestTemplate> {
  if (!relearn) {
    const cached = readTemplate()
...
export const xAdapter: PlatformAdapter = {
  ...
  async fetchPage(cursor: string | null, startRank: number): Promise<NormalizedPage> {
    let tpl = await template()
    let response: XBookmarksResponse

    try {
      response = await requestPage(tpl, cursor)
    } catch (err) {
